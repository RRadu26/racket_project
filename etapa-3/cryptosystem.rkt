...
(define (key n)
  ...
  (map (lambda (x) (modulo x 27)) (foldl (lambda (fu acc) (append acc (list (fu e f)))) '() ( list a1 b1 c1 a2 b2 c2 a3 b3 c3)))
  ))


; TODO
; Implementați o funcție care primește un mesaj (un șir de
; caractere care conține doar litere mici și spații) șikey 1
...
(define (message->codes message)
  (map (lambda (char) (if (= char 32) (- char 32) (- char 96))) (map char->integer (string->list message))))
...
(define (codes->message codes)
  (list->string (map integer->char (map (lambda (char) (if(= char 0) 32 (+ char 96))) codes))))
...
(define (extend-key key size)
  (take (let iter ((m (ceiling (/ size (length key)))) (rez key))
    (if (<= m 1) rez (iter (- m 1) (append rez key)))) size))
...
(define (cript func message key)
  (map (lambda (x y) (modulo (func x y) 27)) message (extend-key key (length message))))
