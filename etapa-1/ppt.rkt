...
(define (multiply M V)
  (multiplyhelp M V '()))
(define (multiplyhelp M V acc)
  (if (null? M) acc
  (multiplyhelp (cdr M) V (append acc (list (dot-product (car M) V))))
  ))
...
(define (apply-matrix-transformations Ts ppt)
  (if (null? Ts) ppt
     (cond ((= 1 (car Ts)) (apply-matrix-transformations (cdr Ts) (multiply T1 ppt))
      )
     ((= 2 (car Ts)) (apply-matrix-transformations (cdr Ts) (multiply T2 ppt))
      )
     ((= 3 (car Ts)) (apply-matrix-transformations (cdr Ts) (multiply T3 ppt))
      )
           )))
